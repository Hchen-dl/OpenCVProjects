...
for Wavelength=401:999
    ...
    %删除之前存在的图像；
     delete(strcat(Leaf_Path_0,'*.jpg'));
     delete(strcat(Leaf_Path_30,'*.jpg'));
     delete(strcat(Leaf_Path_45,'*.jpg'));
     %是否选定波长
     ...
     for index=0:5
         ...
         cla(handles.axes1);
         axes(handles.axes1);
         PlotImage(Leaf_FR, Wavelength,1, Leaf_Path_0);
         %% save image
         axes1 =handles.axes1; 
         if isempty(axes1)
            errordlg('叶片区域没有图像','错误');
         ...
         Leaf_Path=strcat(Leaf_Path_0,tem_index,'_',wavelength_str,'_0.jpg');%_0表示0度角；,'.jpg'
         imwrite(f,Leaf_Path);
     ...
     for index=0:44
         ...
         cla(handles.axes1);
         axes(handles.axes1);
         PlotImage(Leaf_FR, Wavelength,1, Leaf_Path_30);
         %% save image
         axes1 =handles.axes1; 
         if isempty(axes1)
            errordlg('叶片区域没有图像','错误');
         ...
         f = frame2im(f);
         wavelength_str=num2str(Wavelength);
         Leaf_Path=strcat(Leaf_Path_30,tem_index,'_',wavelength_str,'_30.jpg');%_30表示30度角
         imwrite(f,Leaf_Path);         
         set(handles.InfoText,'string',strcat('叶片图像已保存为：',Leaf_Path));        
     end     
%    
     ...
     for index=0:44
         ...
         cla(handles.axes1);
         axes(handles.axes1);
         PlotImage(Leaf_FR, Wavelength,1, Leaf_Path_45);
         %% save image
         axes1 =handles.axes1; 
         if isempty(axes1)
            errordlg('叶片区域没有图像','错误');
         ...
         f = frame2im(f);
         wavelength_str=num2str(Wavelength);
         Leaf_Path=strcat(Leaf_Path_45,tem_index,'_',wavelength_str,'_45.jpg');%_45表示30度角
         imwrite(f,Leaf_Path);
         set(handles.InfoText,'string',strcat('叶片图像已保存为：',Leaf_Path));
     end  
